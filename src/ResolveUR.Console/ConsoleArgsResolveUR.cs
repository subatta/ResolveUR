...
namespace ResolveUR
{
    ...
    class ConsoleArgsResolveUR
    {
        public static ConsoleArgs Resolve(string[] args)
        {
            ...
            var isResolvePackage = args.Length >= 2 && string.Equals(args[1], "true", StringComparison.InvariantCultureIgnoreCase);

            // 3rd arg can be platform - x86 or x64
            var platform = string.Empty;
            if (args.Length >= 3 && (args[2] == Constants.X86 || args[2] == Constants.X64))
                platform = args[2];
